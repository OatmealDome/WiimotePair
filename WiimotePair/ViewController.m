...
- (void)startInquiry
{
  self.attemptingPairing = false;
  
  self.deviceInquiry = [IOBluetoothDeviceInquiry inquiryWithDelegate:self];
  self.deviceInquiry.searchType = kIOBluetoothDeviceSearchClassic;
...
- (void)deviceInquiryDeviceFound:(IOBluetoothDeviceInquiry*)sender device:(IOBluetoothDevice*)device
{
  ...
  if (!device.isPaired && !self.attemptingPairing && [device.name containsString:@"Nintendo"])
  {
    ...
    self.devicePair = [IOBluetoothDevicePair pairWithDevice:self.device];
    self.devicePair.delegate = self;
    
    IOReturn pair = [self.devicePair start];
    if (pair != kIOReturnSuccess)
    {
      NSLog(@"pair start fail");
    }
    
    self.attemptingPairing = true;
  }
}

- (void)devicePairingStarted:(id)sender
{
  NSLog(@"pair start (delegate)");
}
...
{
  NSLog(@"user confirm");
  [self.devicePair replyUserConfirmation:true];
}
...
{
  NSLog(@"pin req");
  IOBluetoothDevicePair* devicePair = (IOBluetoothDevicePair*)sender;
  const BluetoothDeviceAddress* address = [devicePair.device getAddress];
  
  BluetoothPINCode pin;
  pin.data[0] = address->data[5];
  pin.data[1] = address->data[4];
  pin.data[2] = address->data[3];
  pin.data[3] = address->data[2];
  pin.data[4] = address->data[1];
  pin.data[5] = address->data[0];
  
  [devicePair replyPINCode:6 PINCode:&pin];
}
...
- (void)devicePairingFinished:(id)sender error:(IOReturn)error
{
  ...
  NSLog(@"error: %s, %d", getIOReturnText(error), error);
}
